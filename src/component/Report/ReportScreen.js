...
const ReportScreen = ({userAccess}) => {
  ...
  const checkAuthToken = useCallback(async () => {
    ...
    const Parameter =  {
      ...
      conditionString:"",
      checkAvailability: "",
      customQuery:`SELECT p.PK_Report_Id, p.EMPLID, p.EXAM_DT, p.ROOM_NBR, p.EXAM_START_TIME, p.STRM, p.CATALOG_NBR, p.PTP_SEQ_CHAR, p.NAME_FORMAL, p.ADM_APPL_NBR, p.DESCR, p.DESCR2, p.DESCR3, p.Status, p.Attendece_Status, p.EXAM_TYPE_CD, p.created_by, p.updated_by, p.isActive, JSON_ARRAYAGG( JSON_OBJECT( 'PK_CopyId', cm.PK_CopyId, 'FK_ReportId', cm.FK_ReportId, 'EMPLID', cm.EMPLID, 'copyNumber', cm.copyNumber, 'isActive', cm.isActive ) ) AS copyData FROM tbl_report_master p LEFT JOIN tbl_copy_master cm ON p.PK_Report_Id = cm.FK_ReportId WHERE p.EXAM_DT = '${date}' GROUP BY p.PK_Report_Id`,
      };
    const response = await fetch(
      Parameter,
    authToken
    ...
    if (response) {
      console.log("Report Response", response?.[0]);
