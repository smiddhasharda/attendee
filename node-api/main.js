...
if (!secretKey) {
  console.error('JWT secret key is missing. Set it in the environment variable (SECRET_KEY) or provide a default.');
  process.exit(1);
}
  ...
  try {
    ...
    app.post('/api/emailVerify', async (req, res) => {
      try {
        ...
        const [rows] = await pool.query(`SELECT * FROM ${tblName} WHERE ${conditionString}`);
        if (rows.length > 0) {
          const userData = rows[0];
          const otp = Math.floor(100000 + Math.random() * 900000);
          const [updateRows] = await pool.query(`UPDATE ${tblName} SET otp = ?  WHERE user_id = ?`, [otp, userData?.user_id]);
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (rows.length > 0) {
      ...
      const expirationTimestamp = Math.floor(Date.now() / 1000) + (60 * 60 * 24); // 24 hours expiry
      const token = jwt.sign({ id: userData.user_id, username: userData.username, exp: expirationTimestamp }, secretKey);
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('INSERT INTO ?? SET ?', [tblName, req.body]);
    ...
    switch (operation) {
      case 'insert':
        if (Array.isArray(data)) {
          // for multiple data 
          const insertResults = await Promise.all(data.map(async (item) => {
            const [insertRows] = await pool.query(`INSERT INTO ${tblName} SET ?`, item);
            return insertRows;
          }));

          const allSuccessful = insertResults.every(result => result.affectedRows > 0);
          ...
          // for single data
          const [insertRows] = await pool.query(`INSERT INTO ${tblName} SET ?`, data);
        ...
        case 'update':
          if (Array.isArray(data)) {
            // for multiple data
            const updateRows = await Promise.all(data.map(async (item) => {
              const { Id, ...itemWithoutId } = item;
              const [updateRows] = await pool.query(`UPDATE ${tblName} SET ? WHERE ${conditionString}`, [itemWithoutId,Id]);
              return updateRows;
            }));
  
            const allSuccessful = updateRows.every(result => result.affectedRows > 0);
            ...
            // for single data
            const [updateRows] = await pool.query(`UPDATE ${tblName} SET ? WHERE ${conditionString}`, data);
            if (updateRows.affectedRows > 0) {
              return res.json({ message: 'Update successful' });
            } else {
              return res.status(500).json({ error: 'Update failed' });
      ...
      case 'delete':
        const [deleteRows] = await pool.query(`DELETE FROM ${tblName} WHERE ${conditionString}`);
        ...
        break;
      case 'fetch':
        const [selectRows] = await pool.query(`SELECT * FROM ${tblName} ${conditionString ? 'WHERE ' + conditionString : ''}`);
  ...
  try {
    ...
    if (checkAvailability) {
      const promises = data.map(async (item) => {
        const { Id, ...itemWithoutId } = item;
        const [checkRows] = await pool.query(`SELECT * FROM ${tblName} WHERE ${conditionString}`, [Id]);
        ...
        if (checkRows.length > 0) {
          ...
          const [insertRows] = await pool.query(`INSERT INTO ${tblName} SET ?`, itemWithoutId);
      ...
      if (hasError) {
        return res.status(500).json({ error: 'One or more operations failed' });
      }
    ...
    switch (operation) {
      ...
      case 'custom':
        const [customRows] = await pool.query(customQuery);
        return res.json({ message: 'Custom query successful', data: customRows });
      default:
